...
UIDrawableSpeciesGraph::UIDrawableSpeciesGraph(std::vector<Runner> const& runners)
    : runners(runners)
    , count(0)
{}

void UIDrawableSpeciesGraph::draw(wxDC& dc) const
{
    wxSize size = getSize();
    dc.SetPen(wxNullPen);
    dc.DrawRectangle(wxPoint{0,0}, size);

    float   distYRel    = (size.y - 20) * 1.0 / (runners.size() - 1);
...
wxSize UIDrawableSpeciesGraph::getSize() const
{
    return wxSize(400, 400);
}

void UIDrawableSpeciesGraph::tick(bool update)
{
    ++count;
...
void UIDrawableSpeciesGraph::load(std::istream& stream)
{
...
void UIDrawableSpeciesGraph::save(std::ostream& stream) const
{
